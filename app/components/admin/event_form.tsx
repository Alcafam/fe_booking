    ...
    try {
      ...
      if (mode === "edit") {
        response = await axios.put(
          `${API_URL}/api/events/update_event/${formData.id}`,
          formData,
          {
            headers: {
        ...
        response = await axios.delete(
          `${API_URL}/api/events/delete_event/${formData.id}`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("auth_token")}`,
            },
      ...
      setErrors(errorData[0].errors);
      showSwal(errorData[0]);
    }
  };
      ...
      <div>
        ...
        <div className="d-flex">
          ...
          <button
            ...
            onClick={() => onSubmit()} // This will close the form without deleting
          >
            No, Cancel
          </button>
        </div>
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div className="mb-3">
        <label htmlFor="event_time" className="form-label">
          Event Time: {formData.event_time}
        </label>
        <input
          type="time"
          id="event_time"
        ...
        />
        {errors?.event_time && (
          <p className="text-danger">{errors.event_time}</p>
        )}
      </div>
      ...
      <div className="mb-3">
        <label htmlFor="total_capacity" className="form-label">
          Total Capacity:
        </label>
        <input
          type="number"
        ...
        {errors?.total_capacity && (
          <p className="text-danger">{errors.total_capacity}</p>
        )}
      </div>
